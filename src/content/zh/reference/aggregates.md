...
结构体是有序、字段名唯一的名义乘积类型。可以按声明顺序位置构造，或用字段名构造。一次构造必须恰好初始化每个字段一次，不能混合位置和命名形式。空构造仅用于无字段结构体。字段体只容纳字段，没有成员函数定义。
...
命名形式映射到字段声明，但初始化表达式仍按书写顺序求值。重复、遗漏、多余、未知字段和不兼容值都报错。Carven 的 `T { ... }` 仅构造结构体；内建类型、枚举和 callable 使用各自的表达式形式。外部 C++ 类型有单独的构造规则。
