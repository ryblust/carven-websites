...
Carven 编译器使用 C++26，关闭异常与 RTTI。当前验证宿主为 LLVM/Clang 与 libc++ 23.1.0。生成程序和安装 Crafts 的最低标准为 C++20；消费项目选择标准，运行时通过特性检测使用可用设施。宿主要求不提高目标最低标准。

原生消费者根据提供者需求选择异常支持；包含 C++ throw/try/catch 的 cpp 片段需要其翻译单元开启异常。宿主和目标的 isize/usize 数据模型必须相同，f32/f64 要求 IEEE binary32/64，原生选项须保留相等和求值语义。

## 构建责任

成功交付需要 Carven 分析、C++ 编译和链接三步。构建系统提供完整源批次、原生 include 路径、提供者、库和编译选项。`carven compile` 写出产物，直接源码运行模式另外完成本地原生编译与执行。
...
先在 staging 生成，再提升到 live 输出。编译器调用失败保留 live 产物；提升本身可能部分失败。批次、增量提升和恢复由规则仓库负责。
...
export(cpp) 位于自包含 `carven/api` 头的 `carven::api` 下，随后嵌套编码后的模块 namespace，对应 façade 位于实现。C++ 头导入按模块原次序进入所需产物；外部接口类型需要其上下文模块的完整头文件环境。保留分隔符和重复 include，跨模块使用稳定模块顺序，不复刻任意宏配置顺序。cpp 片段位于实现的 include 后、生成 namespace 前。
...
compiler 与 support 必须匹配。私有生成名字、helper 选择、表示布局属于实现细节。一般插值需要 C++20 format 支持；print 可按特性检测使用 C++23 实现而不改变消费者选择的标准。
