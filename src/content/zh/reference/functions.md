...
普通命名函数的每个参数必须有类型，名字唯一。实参个数、访问标记和类型必须匹配。函数不重载，没有默认参数、可变参数、嵌套函数或用户泛型参数列表。带体函数使用块体或 `=> expression;`。

先求值 callee，再从左到右求值实参，每个一次。具体闭包选中对象身份，view 保存目标描述；参数完成后调用目标并读取当前捕获。调用参数中的副作用可能改变后来读取的别名存储。
...
表达式体等价于返回该表达式，使用同样的访问、生命周期和失败消费规则。void 表达式可作为表达式体，也可 `return action();`。可失败表达式须显式 `?`，例如 `return action()?;`。

## 递归与声明顺序

函数身份和头部先收集，支持向后声明引用。需要结果推断的函数按依赖完成。结果推断环产生 `CV-TYPE-RESULT-INFERENCE-CYCLE`，用显式 `-> T` 打断；不能靠调用者上下文、运算符要求或常量分支猜测环内结果。
