---
title: 模块、声明与可见性
description: 显式输入批次、craft 域、导入解析、名字查找和接口可见性。
section: reference
lesson: 2
...
一次编译只包含调用者显式提供的源文件。导入不会自动搜索磁盘或下载依赖。每个输入对应唯一规范模块名；路径分量符合 `[A-Za-z_][A-Za-z0-9_]*`，模块分量允许语言关键字。
...
函数、结构体、枚举和模块常量共享模块命名空间。同名模块声明非法，函数没有重载。模块还可包含测试、C++ 片段和入口语句。
